...
class DistanceExponentialChangeRewardProcessorNew(RewardProcessor):
    ...
    def __init__(self,
        ...
        self.prev_dist = math.inf
        self.curr_dist = math.inf
        self.a2 = math.log(2) / 0.5
        self.a = math.log(2) / 5

    def reset(self, sim_state):
        super().reset(sim_state)

        dist = np.linalg.norm(sim_state.env_objs[self.target].position -
                              sim_state.env_objs[self.agent].position,
                              ord=1)
    ...
    def _increment(self, sim_state, step_size):
        ...
        dist = sim_state.env_objs[self.target].position - \
               sim_state.env_objs[self.agent].position

        self.curr_dist = np.linalg.norm(dist, ord=1)

    def _process(self, sim_state):
        return self.c * ((math.exp(-self.a * self.curr_dist) -
                          math.exp(-self.a * self.prev_dist)) +
                         (math.exp(-self.a2 * self.curr_dist) -
                          math.exp(-self.a2 * self.prev_dist)))

...
class SimpleDockingInitializer(Initializer):
    def get_init_params(self):
        ...
        if np.random.rand() < 0.5:
            x = ref.x + np.random.uniform(radius[0], radius[1]) if np.random.rand() < 0.5 else ref.x - np.random.uniform(radius[0], radius[1])
            y = ref.y + np.random.uniform(-radius[1], radius[1]) 
        else:
            y = ref.y + np.random.uniform(radius[0], radius[1]) if np.random.rand() < 0.5 else ref.y - np.random.uniform(radius[0], radius[1])
            x = ref.x + np.random.uniform(-radius[1], radius[1]) 
        x_dot = 0
        y_dot = 0
        new_params = {
            "x": x, 

...
from CtrlRoomTemp import MIN_TEMP, MAX_TEMP, TEMP_DISC ,CtrlRoomTemp
import tyro
from typing import Optional, Literal
from torch.utils.tensorboard import SummaryWriter

from barrier import plot_function, verification_loop
...
def make_env(env_id, seed, idx, capture_video, run_name, disable_ctrl_cost=False):
    ...
    def thunk():
        if capture_video and idx == 0:
            env = CtrlRoomTemp(render_mode="rgb_array")  # gym.make(env_id, render_mode="rgb_array", **env_kwargs)
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}")
        else:
            env = CtrlRoomTemp()  # gym.make(env_id, **env_kwargs) 
...
class WNNActor(nn.Module):
    def __init__(self, env, args):
        ...
        if args.network_type == "wnn":
            ...
            #print(f"action dim: {env.single_action_space.shape};  {np.prod(env.single_action_space.shape)}")
...
def evaluate_actor(
    ...
    eval_env = CtrlRoomTemp(render_mode="rgb_array" if episodes > 100 else None) #gym.make(env_id)
...
if __name__ == "__main__":
    ...
    plot_function(actor, "controller_plot_before.png", type_wnnactor=True)
    ...
    actor, certificate_nn = verification_loop(actor, certificate_nn)

...
#from barrier import CTRL_INTERVAL, CtrlRoomTemp, TEMP_DISC, CTRL_DISC, transition, MIN_TEMP, MAX_TEMP, train_controller_and_certificate, plot_function
...
def make_env(env_id, seed, idx, capture_video, run_name, disable_ctrl_cost=False):
    def thunk():
        if capture_video and idx == 0:
            env = CtrlRoomTemp(render_mode="rgb_array")
...
class WNNActor(nn.Module):
    def __init__(self, env, args):
        ...
        if args.network_type == "wnn":
            thermo = ThermometerUniform(n_bits=bits - 1, device='cuda') #In the thermometer they divide by bits + 1
...
def evaluate_actor(
    ...
    eval_env = CtrlRoomTemp(render_mode="rgb_array" if episodes > 100 else None)
    eval_env = gym.wrappers.RecordEpisodeStatistics(eval_env)
    if episodes > 100:
        eval_env = gym.wrappers.RecordVideo(
            eval_env,
            video_folder="CtrlRoomTemp",    # Folder to save videos
            name_prefix="eval_" + str(global_step),  # Prefix for video filenames
            episode_trigger=lambda x: x%2000==0    # Record every episode
        )
...
if __name__ == "__main__":
    ...
    for global_step in tqdm(range(args.total_timesteps)):
        ...
        if global_step == args.total_timesteps - 1:
            ...
            print(f"Final evaluation over {args.final_evaluation_episodes} episodes done!")
    
    print("\n********FINE TUNING CONTROLLER AND CERTIFICATE*********\n")
    # plot_function(actor.fc_mean, "controller_plot_before.png")
    # print("Controller plot before fine-tuning saved as controller_plot_before.png")
    # obs_dim = np.array(envs.single_observation_space.shape).prod()
    # thermo = ThermometerUniform(n_bits=args.bits, device='cuda')

    # min_values = torch.ones((obs_dim,)) * MIN_TEMP
    # max_values = torch.ones((obs_dim,)) * MAX_TEMP
    
    # thermo.fit(torch.zeros((1, obs_dim)), min_value=min_values, max_value=max_values)
    # certificate_nn = WNN(obs_dim=np.array(envs.single_observation_space.shape).prod(), 
    #                         act_dim=np.prod(envs.single_action_space.shape), 
    #                         sizes=[500] * 2, 
    #                         thermometer=thermo, bits=args.bits,
    #                         n=args.n,
    #                         later_learnable=True).to(device)
    # print("starting fine-tuning of the controller and certificate")
    # controller, certificate_nn, _ = train_controller_and_certificate(actor, certificate_nn)
    # plot_function(certificate_nn)
    # plot_function(controller, "controller_plot_after.png")
    # print("Controller plot after fine-tuning saved as controller_plot_after.png")
    ...
    if args.network_type == 'wnn':
        if obs_norm is not None:
            torch.save({
                ...
                #"certificate": certificate_nn.state_dict(),
